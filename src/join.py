...
class join(tk.Frame):
    ...
    def load_image(root):
        ...
        root.modal_create_room_image = root.modal_create_room_image.resize(
            (int(root.screen_width*0.3), int(root.screen_height*0.6)), Image.ANTIALIAS)
    ...
    def create_canvas(root):
        ...
        root.background_canvas.create_image(int(root.screen_width * 0.36), int(root.screen_height / 8), anchor=tk.NW, image=root.modal_create_room_photo)
        ...
        root.background_canvas.create_image(int(root.screen_width * 0.46), int(root.screen_height / 3.1), anchor=tk.NW, image=root.room_code_photo)

        root.name_entry = ttk.Entry(root.background_canvas)
        root.name_entry.place(x=int(root.screen_width * 0.44), y=int(root.screen_height / 4))

        create_button = ttk.Button(
        root.background_canvas, image=root.join_room_btn_photo, command=root.join_room)
        create_button.place(x=int(root.screen_width * 0.45), y=int(root.screen_height * 0.5))

        root.code_entry = ttk.Entry(root.background_canvas)
        root.code_entry.place(x=int(root.screen_width * 0.44), y=int(root.screen_height * 0.37))
