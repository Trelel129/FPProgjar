...
class Button:
    ...
    def draw(self, surface):
        ...
        self.text.draw(surface, self.panel.rect[0] + 35, self.panel.rect[1] + 20, textColor)
...
class InputField:
    ...
    def draw(self, surface, panelColor, textColor):
        if self.active:
            ...
            textColor = temp
        self.panel.draw(surface, panelColor)
        self.text.draw(surface, self.panel.rect[0] + 10, self.panel.rect[1] + 20, textColor)

class ViewController:
    def __init__(self):
        self.screen = pg.display.set_mode((800, 600))
        self.palette = {
            "background": (162, 99, 242),
            "dark": (53, 19, 97),
            "purple": (143, 55, 255),
            "light": (199, 178, 224)
        }
...
class ServerSelect(ViewController):
    def __init__(self):
        super().__init__()
        self.screen = pg.display.set_mode((800,600))
        self.IPLabel = Label("Let's Play The Game", pg.font.SysFont("arial", 36))

        submitLabel = Label("Start", pg.font.SysFont("arial", 20))
        submitPanel = Rectangle((350,300), (100,32))
        self.submitButton = Button(submitPanel, submitLabel, self.palette["dark"], self.palette["light"])

        self.ready = False

    def handleClick(self):
        self.submitButton.active = self.submitButton.hasMouse()
        if self.submitButton.hasMouse():
            self.ready = True
    
    def handleButtonPress(self, event):
        if self.submitButton.active:
            self.submitButton.handleKeyPress(event)
    
    def shouldAdvance(self, controller):
        if self.ready:
            portNumber = 25013
            controller.socket.connect(("", portNumber))
            return True
        return False
    ...
    def drawScreen(self, controller):
        ...
        self.IPLabel.draw(self.screen, 250, 250, self.palette["dark"])
...
class ClientLogin(ViewController):
    def __init__(self):
        ...
        namePanel = Rectangle((300,250), (200,32))
    ...
    def drawScreen(self, controller):
        ...
        self.nameField.draw(self.screen, self.palette["dark"], self.palette["light"])
        self.submitButton.draw(self.screen)
...
class ChatRoom(ViewController):
    def __init__(self):
        ...
        messagePanel = Rectangle((20,320), (190,30))
        self.messageField = InputField(messageLabel, messagePanel)

        submitLabel = Label("Send", self.font)
        submitPanel = Rectangle((220,320), (90,30))
        self.submitButton = Button(submitPanel, submitLabel, self.palette["dark"], self.palette["light"])

        self.messagePanel = Rectangle((20,10), (290,300))
    ...
    def drawScreen(self, controller):
        ...
        self.messageField.draw(self.screen, self.palette["purple"], self.palette["light"])
        self.submitButton.draw(self.screen)
        self.messagePanel.draw(self.screen, self.palette["dark"])
        ...
        while messageEntry is not None and y > 10:
            ...
            messageLabel.draw(self.screen, 50, y + 5, self.palette["light"])
            y -= 20
